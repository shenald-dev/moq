...
Action: Secure URL decoding by utilizing a safe `while` loop that decodes recursively up to a strict limit (e.g., 10) as long as `%` characters exist. Avoid allocating arrays with `split('/')` in hot paths by counting slashes (`charCodeAt(0) === 47`) and returning early if there are no candidates for that segment depth.
