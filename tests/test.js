...
async function runTests() {
  ...
  // Test 9: Root path mock resolution
  try {
    const fs = require('fs');
    ...
    if (r.status === 200 && r.body && r.body.root === true) {
      console.log('✅ Root path mock resolution');
      passed++;
    } else {
      console.log('❌ Root path mock resolution failed', r);
      failed++;
    }
  } catch (e) {
    console.log('❌ Root path mock resolution error', e);
    failed++;
  } finally {
    const fs = require('fs');
